...
| 3:30-4:20 | Eval harness | Build Quality | Run eval; show gated 100% injection resistance + 8 V2 metrics |
| 4:20-4:50 | What broke + next | Honesty | Show day08.md What broke section |
| 4:50-5:00 | CTA | — | Repo URL, MIT, 143 tests green |
...
Show the protocol map: buyer agent -> storefront -> gateway (R1-R12) -> Razorpay -> audit chain. First violation wins.

## 1:30-2:30 — Negotiation Engine (Day 5 headline)

Visual: negotiation/engine.py with clamp_offer highlighted. Run a live negotiation.
...
Five arms: static (baseline), ungated (naive, fraud loss Rs 74,861), gated
(SELLABLE, 100% injection resistance, 0% money loss), behavioral_ungated_llm,
behavioral_gated_llm. Headline: gated > ungated > static after fraud loss.

Show `eval/report.json` with all 8 required metrics:
...
Show `python scripts/verify_numbers.py --check-report --check-readme` proving
README numbers match report.json.

## 4:20-4:50 — What Broke (honesty)

Visual: docs/log/day08.md What broke section.

Five things broke, each taught a lesson. Judges read "what broke" first.
(cp1252 console crashes, silent-degradation test deletion, encoding round-trip bugs, eval on Windows).

## 4:50-5:00 — CTA

On-screen: `github.com/HarshDubey23/SELLABLE` — MIT — 143 tests green.
